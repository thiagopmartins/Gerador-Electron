...
class CancelamentoModel {
    ...
    testeCanInu(agentId, destino, timeout) {
        setTimeout(() => {

            let content = null;
            let ext;
            content = `0000;CANCINUT;43180210280765000186652510003525661782074271;Dados incorretos da nota;Dados incorretos da not`;
            ext = '_ped_can-inu.txt';
            console.log(content);
            fs.writeFileSync(`${destino}\\${agentId}#123${ext}`, content);
            this.testeCanInu(agentId, destino, timeout);
        }, +timeout);
    }
